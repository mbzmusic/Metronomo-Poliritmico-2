...
// --- GESTIONE LONG PRESS PER SUDDIVISIONE (DA 1 A 7) ---
let longPressTimer = null;
const LONG_PRESS_DURATION = 400;

function attachLongPressSubdivisionHandler(selectElem, index) {
  ...
  const startPress = (e) => {
    ...
    longPressTimer = setTimeout(() => {
      isLongPress = true;
      showSubdivisionQuickMenu(e, index, selectElem);
    }, LONG_PRESS_DURATION);
  };
  ...
  selectElem.addEventListener('mousedown', startPress);
  selectElem.addEventListener('mouseup', cancelPress);
  selectElem.addEventListener('mouseleave', cancelPress);

  selectElem.addEventListener('touchstart', startPress, { passive: true });
  selectElem.addEventListener('touchend', cancelPress);
  selectElem.addEventListener('touchcancel', cancelPress);
}

function showSubdivisionQuickMenu(e, measureIndex, targetElem) {
  const existingMenu = document.getElementById('subdivisionQuickMenu');
  if (existingMenu) existingMenu.remove();

  const menu = document.createElement('div');
  menu.id = 'subdivisionQuickMenu';
  menu.className = 'quick-subdivision-menu';
  ...
  const options = [
    { val: 1, label: '1 - Quarti' },
    { val: 2, label: '2 - Crome' },
    { val: 3, label: '3 - Terzine' },
    { val: 4, label: '4 - Quartine' },
    { val: 5, label: '5 - Quintine' },
    { val: 6, label: '6 - Sestine' },
    { val: 7, label: '7 - Settimine' }
  ];

  options.forEach(opt => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'quick-sub-btn';
    if (measures[measureIndex].sub === opt.val) btn.classList.add('active');
    btn.innerText = opt.label;
    
    btn.onclick = (event) => {
      event.stopPropagation();
      updateMeasure(measureIndex, 'sub', opt.val);
      menu.remove();
    };
  ...
  measures.forEach((m, index) => {
    ...
    row.innerHTML = `
      <div class="measure-left">
        ...
        <select class="measure-select preset" id="presetSelect_${index}" onchange="handlePresetSelect(${index}, this.value)" onclick="handlePresetClick(${index}, this)" aria-label="Metro">
        ...
        <select class="measure-select subdivision" id="subSelect_${index}" onchange="updateMeasure(${index}, 'sub', this.value)" aria-label="Suddivisione">
          ...
          <option value="7" ${m.sub === 7 ? 'selected' : ''}>Settimine</option>
        </select>
    ...
    const presetSelect = document.getElementById(`presetSelect_${index}`);
    if (presetSelect) attachLongPressSubdivisionHandler(presetSelect, index);

    const subSelect = document.getElementById(`subSelect_${index}`);
    if (subSelect) attachLongPressSubdivisionHandler(subSelect, index);
  });
